...
![tvision 动图——菜单栏、一次智能体回合、五套皮肤](docs/screenshots/demo.gif)
...
- **工具断点**——把调试器的断点搬到智能体身上：`/breakpoint bash(rm *)` 让匹配的调用在运行前停进模态框——`Y` 运行一次、`a` 本会话总是、`n` 拒绝（模型会被告知）。`--deny` 规则不问直接拒。`Ctrl+B` 管理规则；模式匹配的正是转录里显示的那行标签。
